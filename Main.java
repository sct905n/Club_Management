...
class ClubApp {
    ...
    public ClubApp(ClubApp another){
        this(new ArrayList<Member>(another.getMembers()), another.member_fee, another.hall_rent, another.coach);
    }
    ...
    public int getRevenue(int month_num) {
        if (1 <= month_num && month_num <= 12) {
            ...
            for (int i = 0; i < member_list.size(); i++) {
                ...
                total_attendance += current.getMonthlyInattendance()[month_num-1];
    ...
    public void sortByTimesAttended() {
        Collections.sort(member_list, new Comparator<Member>() {
            @Override
            public int compare(Member m1, Member m2) {
                return Integer.compare(m2.getTimesAttended(), m1.getTimesAttended());
            }
        });
    }

    public void setLoyatly() {
        sortByTimesAttended();

        int i = 0;
        while (i < 10) {
            member_list.get(i).updateDiscountStatus(true);
            i++;
        }
    }

    public void checkConsecutivePayment() {
        for (Member member : member_list) {
            member.checkDiscountEligibility();
        }
    }
    ...
    public void updateMemberList() {
        setLoyatly();
        checkConsecutivePayment();

        for (Member current : member_list) {
            current.UnpaidReminder();
        }
    }
...
class Coach {
    ...
    public int[] getMonthlyInattendance() {
    ...
    public void setMonthlySalary(int month_num, int salary){
        if (1 <= month_num && month_num <= 12) {
            this.monthly_salary[month_num-1] = salary;
        }
    }
...
class Member implements Comparable<Member> {
    ...
    public Member(int id, String first, String last, String phone, String address, int[] monthly, int attended, 
        ...
        for (String month : months) {
            ...
            for (String x : months_unpaid) {
                if (month.equals(x)) {
                    isPaid = true; 
                    break; 
                }
            }
            payments.put(month, isPaid);
        }
    }
    public Member() {
        this.id = 0;
    ...
    public void updateDiscountStatus(boolean x) {
        discount = x;
    }

    public void makePayment(String month) {
        payments.put(month, true);
    }

    public boolean checkDiscountEligibility() {
        int consecutivePayments = 0;
        for (boolean paid : payments.values()) {
            if (paid) {
                consecutivePayments++;
    ...
    public void UnpaidReminder(){
        if (this.times_unpaid > 1){
            System.out.println(first_name + " " + last_name + " still have payments due!");
        }
    }
